...
<div id="myFilter" class="fixed z-30 inset-x-0 top-10 hidden">
  <form action="{{ route('transaction.index') }}" method="GET" class="flex flex-col items-center bg-white mx-auto rounded-lg shadow-xl pt-4 pb-1 px-8 border border-gray-700 overflow-y-auto" style="max-height: 80vh; max-width: 40rem;">
    @csrf
    ...
    <div class="w-full flex max-sm:flex-col sm:mx-auto gap-5 border border-dark">
      ...
      <div class="min-sm:w-1/2 sm:w-full">
        <div class="mt-2 w-full">
          ...
          <select id="small" class="block w-full px-2 py-2 shadow text-sm border border-gray-600 rounded hover:ring-gray-600 hover:ring-1 focus:ring-indigo-700 focus:ring-offset-2" name="item[]" size="6" multiple>
            @foreach($item as $rs)
    ...
    <div class="py-5 flex gap-4">
      <button type="button" id="closeFilter" class="px-3 py-1 bg-red-700 rounded  text-slate-200">Close</button>
      <input type="submit" class="px-3 py-1 bg-blue-700 rounded text-slate-200 cursor-pointer" value="Apply">
    </div>
