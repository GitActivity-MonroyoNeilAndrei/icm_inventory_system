...
<div id="myModal" class="fixed z-30 inset-x-0 top-10">
    <form action="{{ route('item.update', $item->id) }}" method="post" class="flex items-center justify-center">
      @csrf
      @method('PUT')
        <div class="bg-white p-6 rounded-lg shadow-lg w-80 overflow-y-auto" style="max-height: 85vh;">
            ...
            <div class="mt-2 w-full">
                ...
                <input name="name" type="text" class="px-3 w-full py-1 shadow ring-1 ring-gray-600 hover:ring-2 rounded focus-outline-none focus:ring-offset-2 focus:ring-indigo-600" value="{{ $item->item_id }}" required>
            </div>
            ...
            <div class="py-5 flex justify-center gap-4">
                <a class="px-3 py-1 bg-red-700 rounded  text-slate-200" href="{{ route('item.index') }}">Cancel</a>
